...
Namespace GridViewDetailColumnWidth
	Public Class MyGridView
		...
		Private Sub MatchColumnLayout(ByVal otherView As MyGridView)
			...
			Dim viewInfo = If(otherIsDetail, CType(otherView.GetViewInfo(), MyGridViewInfo), CType(Me.GetViewInfo(), MyGridViewInfo))
			...
			If otherIsDetail Then
				otherView.VisibleColumns(0).Width = Me.VisibleColumns(0).Width - viewInfo.LevelIndent - (viewInfo.LevelIndent * Me.GroupCount) - viewInfo.ViewRects.IndicatorWidth - borderWidth + (If(Me.VisibleColumns(0).Fixed = FixedStyle.Left, Me.FixedLineWidth, 0))
			Else
				If Me.VisibleColumns(0) IsNot Nothing Then
					otherView.VisibleColumns(0).Width = Me.VisibleColumns(0).Width + viewInfo.LevelIndent + (viewInfo.LevelIndent * otherView.GroupCount) + viewInfo.ViewRects.IndicatorWidth + borderWidth - (If(otherView.VisibleColumns(0).Fixed = FixedStyle.Left, otherView.FixedLineWidth, 0))
				End If
			End If
		...
		Private Sub UpdateDetailViewsColumnWidths(ByVal excludeView As MyGridView)
            ...
            Dim rowIndex As Integer = TopRowIndex
		...
		Private Sub UpdateParentViewColumnWidths()
			Dim parentView = TryCast(Me.ParentView, MyGridView)
		...
		Protected Overrides Sub DoLeftCoordChanged(ByVal allowPartialScroll As Boolean)
			MyBase.DoLeftCoordChanged(allowPartialScroll)
		...
		Private Sub UpdateParentViewLeftCoord()
            Dim aParentView = TryCast(ParentView, GridView)
            If aParentView Is Nothing Then
				Return
			End If
            aParentView.LeftCoord = LeftCoord
